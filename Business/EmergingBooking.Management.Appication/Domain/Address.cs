...
namespace EmergingBooking.Management.Appication.Domain
{
    internal class Address : ValueObject
    {
        ...
        public static Outcome<Address> Create(PossibleBe<string> street,
                                              ...
                                              PossibleBe<int> zipCode)
        {
            if (IsValidAddress(street, district, city, country, zipCode))            
                return Outcome.Successfully(new Address(street.Value, district.Value, city.Value, country.Value, zipCode.Value));
            
            return Outcome.Failed<Address>("The Address is invalid. Please verify all address values!");
        }

        private static bool IsValidAddress(PossibleBe<string> street, PossibleBe<string> district, PossibleBe<string> city, PossibleBe<string> country, PossibleBe<int> zipCode)
        {
